...
impl Population {
    ...
    fn compatible(&self, person_1: &Human, person_2: &Human) -> bool {
        ...
        person_1.family != person_2.family &&       // person 1 and person 2 are from different families
        person_1.get_valid_spouse_ages().is_some() &&     // person 1 is old enough
        person_1.age >= person_2_ages.0 &&          // person 1 is not too young for person 2
        person_1.age <= person_2_ages.1 &&          // person 1 is not too old for person 2
        self.has_spouses(person_1.id) &&            // person 1 has no spouse
        self.has_spouses(person_2.id)               // person 2 has no spouse
    }
    ...
    pub fn meetups(&mut self) {
        ...
        for _ in 0..self.get_living().len() {
            let people: Vec<&Human> = self.people
                .iter()
